...
int binder_become_context_manager(struct binder_state *bs)
{
    ...
    int result = ioctl(bs->fd, BINDER_SET_CONTEXT_MGR, &obj);

    // fallback to original method
    /*if (result != 0) {
        printf("binder_become_context_manager_ext error");
...
void binder_loop(struct binder_state *bs, binder_handler func)
{
    ...
    for (;;) {
        ...
        if (res < 0) {
            printf("binder_loop: ioctl failed (%s)\n", strerror(errno));
            break;
        }

        res = binder_parse(bs, 0, (uintptr_t) readbuf, bwr.read_consumed, func);
        if (res == 0) {
            printf("binder_loop: unexpected reply?!\n");
            break;
        }
        if (res < 0) {
            printf("binder_loop: io error %d %s\n", res, strerror(errno));
            break;
        }
    }
}
